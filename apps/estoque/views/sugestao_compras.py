"""Módulo de Sugestão de Compras, giro, demanda, cobertura e reposição inteligente."""
...
class SugestaoComprasView(PermissaoRequiredMixin, View):
    ...
    def _qs_produtos(self, filial, filtros: dict):
        ...
        if filtros["busca"]:
            q = filtros["busca"]
            qs = qs.filter(
                Q(descricao__icontains=q) | Q(codigo_barras__icontains=q) | Q(codigo__icontains=q)
            )
        return qs.order_by("descricao")

    # -- cálculos de giro (saídas) -------------------------------------------
    ...
    # -- último custo (última entrada NF) ------------------------------------
